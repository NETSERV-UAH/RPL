...
#include "os/net/ipv6/uip-ds6-route.h"
...
static void print_resul(rtimer_clock_t conv_time){
  ...
  int i,count_addr=0,count_nbr=0;
...
PROCESS_THREAD(mote_process, ev, data)
{
  ...
  while(1) {
    ...
    /* Add some jitter */
    etimer_set(&periodic_timer, 4*CLOCK_SECOND/*SEND_INTERVAL - CLOCK_SECOND + (random_rand() % (2 * CLOCK_SECOND))*/);
