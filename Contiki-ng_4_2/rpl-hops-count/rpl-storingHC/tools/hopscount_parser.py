...
# 
# get_random_seed(file_to_parse)                - returns seed in str                       - needs file descriptor 
# motes_count_per_file(file_to_parse)           - returns number of motes in one scenario   - needs file descriptor
# read_ip_modes(file_to_parse)                  - returns a dic { id_mote : "ipv6(fd00)"}   - needs file descriptor
# get_ipv6_local(ipv6_link_local)               - returns a ipv6 str (fe80::) to (fd00::)   - needs ipv6 str (fe80)
# get_state_of_sim(file_to_parse)               - returns True|False(TEST OK|TEST FAILED)   - needs file descriptor
# hops_count_per_file(file_to_parse,dic_motes)  - returns nothing(reads M[id]-M[id]: hops)  - needs file descriptor and dic_motes
# complete_hops_allWall_ns(...)                 - returns True|False(if sim_conv)           - needs file_to_parse, dic_motes, dic_motes_ip, sim_stats, is_a_no_conv_sim,dic_default_routes
# add_stats_to_results_file(sim_stats,path)     - returns nothing(create ParserResult.txt)  - needs sim_stats, path
# use_ip_hops_check(...)                        - returns True|False(if bool_sim_conv)      - needs file_to_parse, dic_motes, dic_ip, id_mote
# get_node_id_from_ipv6(dic_motes_ip, ipv6)     - returns node_id str                       - needs ipv6(fd00) str and dic_motes_ip
# read_routes_per_motes(...)                    - returns a routing dic                     - needs file_to_parse, num_motes, dic_motes_ip
# complete_hops_allWall_s(...)                  - returns nothing, complete dic_motes       - needs file_to_parse, dic_motes, dic_motes_ip, sim_stats, dic_routing_tables
# write_parsed_data(...)                        - returns nothing, writes parsed into file  - needs file_parsed, dic_motes, file, num_motes
# orderLogDir(path, sim_stats)                  - return nothing, just order parsed data    - needs path, sim_stats
# manage_not_converged_sim(...)                 - returns nothing, just rename no conv sim  - needs file_to_parse,   str path,  str result_dir
# read_default_routes_per_mote_ns(...)          - returns default routes dic                - needs file_to_parse, dic_motes_ip
            ...
            if(file.count('scriptlog')):
                ...
                try:
                    ...
                    if args.nonstoring:
                        ...
                        #Read default routes
                        dic_default_routes = read_default_routes_per_mote_ns(file_to_parse, dic_motes_ip)

                        try: # Stats( Check if this simulation is:  OK / FAILED )
                            if get_state_of_sim(file_to_parse) :
                        ...
                        try:  
                            ...
                            sim_conv = complete_hops_allWall_ns(file_to_parse,dic_motes,dic_motes_ip, sim_stats, is_a_no_conv_sim,dic_default_routes)
                            
                            #if the simulation didnt converge, just log it
                            if not sim_conv:
                                log_parser.write('Error: Not converged \n')
                        except:
                            print('Error, cannot complete hops all with all: '+ file +'\n')
...
def get_state_of_sim(file_to_parse):
...
def hops_count_per_file(file_to_parse,dic_motes):
    ...
    for line in file_to_parse:
        if(line.count('M[') and (not line.count('routes')) and (not line.count('through'))):
...
def complete_hops_allWall_ns(file_to_parse,dic_motes, dic_ip, sim_stats, is_a_no_conv_sim,dic_default_routes):
    bool_sim_conv = True
    ...
    #Complete all posible paths using double check for hops to sink and default routes
    #A David del futuro: Hay que modificar esta parte -> meter algoritmo que tienes en cuadernito azul
    for src_mote in range(2, motes_count_per_file(file_to_parse) + 1):
        for dst_mote in range(2, motes_count_per_file(file_to_parse) +1):
            if src_mote != dst_mote:

                #Obtenemos el next hop gracias nuestra ruta por defecto y aumentamos a 1 los hops
                next_hop = dic_default_routes[src_mote]
                hops += 1

                #Mientras que el nexthop no sea el destino final 
                while not (next_hop == dic_ip[dst_mote]):

                    #Mira el siguiente salto desde la mota siguiente
                    next_hop = dic_default_routes[int(get_node_id_from_ipv6(dic_ip,next_hop))]

                    #Si el siguiente salto es el padre, suma la distancia en saltos hacia la mota
                    # destino desde el padre
                    if next_hop is 'father':
                        hops += int(dic_motes[1][str(dst_mote)])
                        break
                    
                    hops += 1
                    
                #Guardamos los saltos calculados y hops == 0
                dic_motes[src_mote][str(dst_mote)] = str(hops)
                hops = 0
...
#Read default routes in non-storing sim, returns default routes dic
def read_default_routes_per_mote_ns(file_to_parse, dic_motes_ip):
    #Aux. vars
    dic_default_routes = {0 : 'Nothing here'}

    for i in range(1,motes_count_per_file(file_to_parse)+1):
        if i is 1:
            dic_default_routes[i]  = 'father'
        else:
            dic_default_routes[i]  = { i : '-'}

    for line in file_to_parse:
        if line.count('through'):

            #Get the node_id M[node_id]
            node_id = int((line.split('M[')[1]).split(']')[0])

            #Get default route M[5]: fe80::212:7401:1:101 through fe80::212:7401:1:101\n
            #And parse to fd00::
            #Store in our dic 
            dic_default_routes[node_id] = get_ipv6_local((line.split('through ')[1][:len(line.split('through ')[1]) - 1 ]))

    file_to_parse.seek(0)
    return dic_default_routes
