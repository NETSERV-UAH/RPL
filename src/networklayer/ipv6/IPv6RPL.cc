...
//EXTRA
#include "src/networklayer/ipv6/IPv6RPL.h"
...
#ifdef WITH_xMIPv6
IPv6RPL::ScheduledDatagram::ScheduledDatagram(IPv6Datagram *datagram, const InterfaceEntry *ie, MACAddress macAddr, bool fromHL) :
        datagram(datagram),
        ie(ie),
        macAddr(macAddr),
        fromHL(fromHL)
...
IPv6RPL::ScheduledDatagram::~ScheduledDatagram()
{
    delete datagram;
}
#endif /* WITH_xMIPv6 */

void IPv6RPL::initialize(int stage)
{
...
void IPv6RPL::refreshDisplay() const
{
    char buf[80] = "";
    if (numForwarded > 0)
...
void IPv6RPL::preroutingFinish(IPv6Datagram *datagram, const InterfaceEntry *fromIE, const InterfaceEntry *destIE, IPv6Address nextHopAddr)
{
...
IPv6Datagram *IPv6RPL::encapsulate(cPacket *transportPacket, IPv6ControlInfo *controlInfo)
{
...
void IPv6RPL::fragmentAndSend(IPv6Datagram *datagram, const InterfaceEntry *ie, const MACAddress& nextHopAddr, bool fromHL)
{
    // hop counter check
...
void IPv6RPL::sendDatagramToOutput(IPv6Datagram *datagram, const InterfaceEntry *destIE, const MACAddress& macAddr)
{
...
bool IPv6RPL::determineOutputInterface(const IPv6Address& destAddress, IPv6Address& nextHop,
...
#ifdef WITH_xMIPv6
bool IPv6RPL::processExtensionHeaders(IPv6Datagram *datagram)
{
...
bool IPv6RPL::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    throw cRuntimeError("Lifecycle operation support not implemented");
}

// NetFilter:
void IPv6RPL::registerHook(int priority, INetfilter::IHook *hook)
{
    Enter_Method("registerHook()");
    hooks.insert(std::pair<int, INetfilter::IHook *>(priority, hook));
}

void IPv6RPL::unregisterHook(int priority, INetfilter::IHook *hook)
{
...
void IPv6RPL::dropQueuedDatagram(const INetworkDatagram *datagram)
{
...
void IPv6RPL::reinjectQueuedDatagram(const INetworkDatagram *datagram)
{
...
INetfilter::IHook::Result IPv6RPL::datagramPreRoutingHook(INetworkDatagram *datagram, const InterfaceEntry *inIE, const InterfaceEntry *& outIE, L3Address& nextHopAddr)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result IPv6RPL::datagramForwardHook(INetworkDatagram *datagram, const InterfaceEntry *inIE, const InterfaceEntry *& outIE, L3Address& nextHopAddr)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result IPv6RPL::datagramPostRoutingHook(INetworkDatagram *datagram, const InterfaceEntry *inIE, const InterfaceEntry *& outIE, L3Address& nextHopAddr)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result IPv6RPL::datagramLocalInHook(INetworkDatagram *datagram, const InterfaceEntry *inIE)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result IPv6RPL::datagramLocalOutHook(INetworkDatagram *datagram, const InterfaceEntry *& outIE, L3Address& nextHopAddr)
{
    for (auto & elem : hooks) {
